...
export const MainDisplay = memo((props: any) => {
  ...
  const [layout, setLayout] = useState<LayoutConfig>(layoutConfig);
        ...
        <>
          ...
          <div className="flex flex-row items-center">
            <SquareSplitHorizontal
              className={iconStyle}
              onClick={() => {
                changeLayout(child, "horizontal", "add");
              }}
            />
            ...
            <SquareSplitVertical
              className={iconStyle}
              onClick={() => {
                changeLayout(child, "vertical", "add");
              }}
            />
            {child.parentId && (
              <>
                ...
                <Trash2
                  className={iconStyle}
                  onClick={() => {
                    changeLayout(child, child.style, "remove");
                  }}
                />
              </>
            )}
...
const WidgetSelection = (props: any) => {
  ...
  const handleExtensionChange = async (value: string) => {
    ...
    setLayout((prevLayout: LayoutConfig) => {
      ...
      const currentPanel = findPanelById(copy, selectedPanel.id);
      if (currentPanel === null) return copy;
      // add the new extension to the children of the selected panel
      currentPanel.content = value.includes("wires") ? (
        Extension
      ) : (
        <Extension />
      );
